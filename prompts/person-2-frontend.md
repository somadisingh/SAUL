...
Keep unsent text if the request fails. On timeout/502/503 refetch the case: the server may have saved the message before the AI failed. Compare client submission to returned message/history or track the pending ID locally; avoid submitting a duplicate. A Retry of the same attempt reuses its clientMessageId. On stale follow-up 409, refetch and show the updated question before resending. Disable sends during any case mutation; do not let switching questions change the payload of an already initiated send.
